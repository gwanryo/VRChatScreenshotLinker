...
namespace VRCX
{
    internal static class ScreenshotHelper
    {
        ...
        public static List<ScreenshotMetadata> FindScreenshots(string query, string directory, ScreenshotSearchType searchType)
        {
            ...
            {
                var metadata = GetScreenshotMetadata(file, false);
                ...
                switch (searchType)
                {
                    ...
                    case ScreenshotSearchType.WorldName:
                        if (!string.IsNullOrEmpty(metadata.World.Name) &&
                            metadata.World.Name.Contains(query, StringComparison.OrdinalIgnoreCase))
                            result.Add(metadata);
                        break;
